...
DevPicture::DevPicture(Mat _image)
{
	image = _image;
	rows = image.rows;
	cols = image.cols;
...
DevPicture::DevPicture(string path)
{
	image = imread(path);
	rows = image.rows;
	cols = image.cols;
...
Mat DevPicture::getHistogram(uint histSizeX, uint histSizeY, uint channel)
{
	std::vector<Mat>	bgr;
	int					histSize = 256;
	float				range[] = { 0, 256 };
...
void DevPicture::resize(uint targetWidth)
{
...
bool DevPicture::adaptControl(HWND hwnd, string name)
{
